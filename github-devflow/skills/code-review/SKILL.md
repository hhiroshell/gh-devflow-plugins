...
If the diff is very large (more than 100 changed files), warn the user that the review may be slow and ask whether to proceed.
...
4. Post the review:
   ```bash
   bash ${CLAUDE_PLUGIN_ROOT}/skills/code-review/scripts/post-review.sh $PR_NUMBER /tmp/review-body.md /tmp/review-comments.json
   ```
